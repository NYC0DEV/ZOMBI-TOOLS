...
class ZombiUTrainer:
    def __init__(self):
        ...
        self.available_keys = ['shift', 'f', 'tab', 'w', 'r', 'space']
...
class CirclePlacer:
    ...
    def check_mouse_clicks(self):
        def on_click(x, y, button, pressed):
            if not self.active or not self.see_through_mode or not self.placing_mode:
                return
                    ...
                    if keyboard.is_pressed('x'):
                        ...
                        if current_time - self.last_click_time > 0.3:
                            ...
                            if self.current_circle < 12:
                                try:
                                    # Use window.after to place circle from main thread
                                    if self.window:
                                        self.window.after(0, lambda: self.place_circle_at_position(x, y))
                                except:
                                    pass
                    else:
            ...
            try:
                ...
                if x_pressed and not self.x_pressed:
                    ...
                    if current_time - self.x_last_press_time < 0.3:
                        ...
                        # Use window.after to update GUI from main thread
                        if self.window:
                            try:
                                self.window.after(0, self._update_see_through_mode)
                            except:
                                pass
                ...
                if self.see_through_mode and self.window and self.placing_mode and self.current_circle < 12:
                    try:
                        mouse_x, mouse_y = pyautogui.position()
                        # Use window.after to update canvas from main thread (non-blocking)
                        if self.window:
                            self.window.after(0, lambda mx=mouse_x, my=mouse_y: self.update_preview_circle(mx, my))
                    except:
                        pass
                elif not self.see_through_mode:
                    try:
                        if self.window:
                            self.window.after(0, self.hide_preview_circle)
                    except:
                        pass
            except Exception as e:
                pass
    ...
    def _update_see_through_mode(self):
        """Update see-through mode UI from main thread (non-blocking)"""
        try:
            if not self.window:
                return
    ...
    def undo_last_circle(self):
        ...
        self.redo_stack.append((last_circle, last_pos))
    ...
    def redo_last_circle(self):
        if not self.redo_stack:
            return
        circle_parts, position = self.redo_stack.pop()
        self.circles.append(circle_parts)
...
class TrainerGUI:
    ...
    def reset_keypad_solver(self):
        """Reset keypad solver with confirmation - disabled during active operations"""
        ...
        if self.engine.autostart_timer_running:
            messagebox.showerror("Cannot Reset", 
                                "⚠ Auto-start timer is running!\n\n"
                                "Wait for timer to finish or press F1\n"
                                "Then you can reset keypad data.")
            self.engine.status = "⚠ Stop timer before resetting!"
            return
        
        if self.engine.timer_active:
            messagebox.showerror("Cannot Reset", 
                                "⚠ Timer is running!\n\n"
                                "Stop the timer first (press F3)\n"
                                "Then you can reset keypad data.")
            self.engine.status = "⚠ Stop timer before resetting!"
            return
    ...
    def add_custom_key_blocker(self):
        """Add custom keys to block from text entry (NOT preset keys)"""
        ...
        # Prevent adding preset keys to custom list
        if key in self.engine.available_keys:
            self.engine.status = f"'{key.upper()}' is a preset key - use preset buttons instead"
            self.entry_custom_keys.delete(0, tk.END)
            return
        
        if key in self.engine.blocked_keys:
            self.engine.status = f"{key.upper()} already blocked"
            # Keep textbox visible, just clear it
            self.entry_custom_keys.delete(0, tk.END)
            return
...
"""
    ...
    def update(self):
        ...
        # Disable Place Circle button and Reset Keypad button during active operations
        # Activities: auto-clicker, auto-start timer, or manual timer running
        if (self.engine.autostart_timer_running or 
            self.engine.auto_clicking or 
            self.engine.timer_active):
